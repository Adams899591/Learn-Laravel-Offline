...
const lessonContent = [
  { 
    ...
    icon: 'flash', 
    color: '#3B82F6', 
    content: 'Events provide a simple observer pattern implementation, allowing you to subscribe and listen to various occurrences happening across your application lifecycle.',
    code: '// Events act as a point of decoupling between different system domains'
  },
  { 
    id: '2', 
    ...
    icon: 'create', 
    color: '#10B981', 
    content: 'Event classes are typically stored in `app/Events` and are simple data containers holding information related to the action that occurred.',
    code: '# Generate an event class via Artisan:\nphp artisan make:event OrderShipped'
  },
  { 
    id: '3', 
    ...
    iconName: 'ear', 
    color: '#8B5CF6', 
    content: 'Listeners accept an event instance in their `handle` method and perform any business logic responses, such as sending emails or updating metrics.',
    code: '# Generate an event listener class:\nphp artisan make:listener SendShipmentNotification --event=OrderShipped'
  },
  { 
    id: '4', 
    ...
    icon: 'paper-plane', 
    color: '#F59E0B', 
    content: 'You can fire events throughout your code using the global `event` helper function or by calling the static `dispatch` method on the event class.',
    code: 'use App\\Events\\OrderShipped;\n\n// Fire the event instance\nevent(new OrderShipped($order));\n// Or static dispatch\nOrderShipped::dispatch($order);'
  },
  { 
    id: '5', 
    ...
    icon: 'search', 
    color: '#06B6D4', 
    content: 'Laravel can automatically discover and register events and listeners by inspecting your `app/Listeners` directory based on type-hinted parameters.',
    code: '// Enabled by default in EventServiceProvider or automatically discovered'
  },
  { 
    id: '6', 
    ...
    icon: 'timer', 
    color: '#F97316', 
    content: 'If a listener performs heavy operations like API requests, you can implement the `ShouldQueue` contract so Laravel dispatches it to the background queue automatically.',
    code: 'use Illuminate\\Contracts\\Queue\\ShouldQueue;\n\nclass SendShipmentNotification implements ShouldQueue {\n    // Listener runs in the background queue\n}'
  }
];
...
export default function Module22DetailScreen() { 
  ...
  const handleReadAloud = () => {
    if (isSpeaking) {
      ...
      setIsSpeaking(true);
      Speech.speak("Module 22: Events and Listeners. Learn observer patterns, event dispatching, and asynchronous listeners in Laravel.", { rate: 0.9 });
        ...
        <SafeAreaView>
          <View className="pt-4 pb-10 px-6">
            <View className="flex-row items-center justify-between mb-6">
              ...
              <Text className="text-white/80 text-sm font-semibold uppercase tracking-wider">Module 22</Text>
              <View className="w-10 h-10" />
            </View>
            <View>
              <Text className="text-white text-3xl font-extrabold tracking-wide mb-2">Events & Listeners</Text>
              <Text className="text-blue-100 text-sm leading-relaxed mb-4">Learn application event broadcasting, decoupled listeners, and queued background event handling.</Text>
              <View className="flex-row items-center space-x-3">
                <View className="flex-row items-center bg-black/10 px-3 py-1.5 rounded-lg border border-white/10">
                  <Ionicons name="flash" size={14} color="#DBEAFE" />
          ...
          <View key={lesson.id} className="bg-white p-5 mb-6 rounded-3xl border border-gray-100 shadow-sm">
            <View className="flex-row items-center mb-3">
              <View className="w-10 h-10 rounded-full items-center justify-center mr-3" style={{ backgroundColor: `${lesson.color}15` }}>
                <Ionicons name={lesson.iconName || lesson.icon} size={20} color={lesson.color} />
              </View>
