import React, { useState, useEffect } from 'react';
import { View, Text, ScrollView, SafeAreaView, TouchableOpacity, StatusBar } from 'react-native';
import { Ionicons } from '@expo/vector-icons';
import { router } from 'expo-router'; 
import * as Clipboard from 'expo-clipboard';
import * as Speech from 'expo-speech';
import { useSafeAreaInsets } from "react-native-safe-area-context";

const lessonContent = [
  { 
    id: '1', 
    title: 'Mail Configuration', 
    icon: 'mail', 
    color: '#3B82F6', 
    content: 'Laravel provides a clean API over the popular Symfony Mailer component. Mail settings (SMTP, Mailgun, Postmark, SES, or log driver for local testing) are configured in your `.env` and `config/mail.php` files.',
    code: '// Environment config (.env):\nMAIL_MAILER=smtp\nMAIL_HOST=smtp.mailtrap.io\nMAIL_PORT=2525\nMAIL_USERNAME=null\nMAIL_PASSWORD=null'
  },
  { 
    id: '2', 
    title: 'Creating Mailables', 
    icon: 'create', 
    color: '#10B981', 
    content: 'In Laravel, email types are represented as "Mailable" classes stored in `app/Mail`. These classes handle passing data, attaching files, and structuring email headers.',
    code: '# Generate a new Mailable class via Artisan:\nphp artisan make:mail OrderShippedMail'
  },
  { 
    id: '3', 
    title: 'Sending Emails', 
    icon: 'send', 
    color: '#8B5CF6', 
    content: 'To send an email, use the `Mail` facade. You specify the recipient email address using the `to` method and pass an instance of your Mailable class to the `send` or `queue` methods.',
    code: 'use App\\Mail\\OrderShippedMail;\nuse Illuminate\\Support\\Facades\\Mail;\n\n// Send email immediately\nMail::to($request->user())->send(new OrderShippedMail($order));'
  },
  { 
    id: '4', 
    title: 'Markdown Emails', 
    icon: 'document-text', 
    color: '#F59E0B', 
    content: 'Laravel allows you to write mailable templates using Markdown syntax. This lets you generate responsive, professional-looking email layouts with built-in buttons and panels easily.',
    code: '// Inside OrderShippedMail build method:\npublic function build() {\n    return $this->markdown(\'emails.orders.shipped\')\n                ->subject(\'Your Order Has Shipped!\');\n}'
  },
  { 
    id: '5', 
    title: 'Email Templates', 
    icon: 'browsers', 
    color: '#06B6D4', 
    content: 'You can design rich HTML views using Blade templates inside `resources/views/emails` or customize published markdown components to match your exact brand colors and typography.',
    code: '{{-- resources/views/emails/orders/shipped.blade.php --}}\n<x-mail::message>\n# Order Shipped\n\nYour order ##{{ $order->id }} has been dispatched.\n\n<x-mail::button :url="\'https://example.com/orders\'">\nView Order\n</x-mail::button>\n\nThanks,\n{{ config(\'app.name\') }}\n</x-mail::message>'
  },
  { 
    id: '6', 
    title: 'Queued Emails', 
    icon: 'time', 
    color: '#F97316', 
    content: 'Sending emails synchronously can slow down HTTP response times for your mobile or web users. By implementing the `ShouldQueue` interface on your Mailable, Laravel automatically pushes emails to a background queue worker.',
    code: 'use Illuminate\\Contracts\\Queue\\ShouldQueue;\n\nclass OrderShippedMail extends Mailable implements ShouldQueue\n{\n    use Queueable;\n\n    // This mail will now dispatch to background workers automatically\n}'
  }
];

const renderHighlightedCode = (codeText) => {
  if (!codeText) return null;
  const regex = /(?:\/\/.*|--.*|#.*)|(?:["'].*?["'])|(?:\$[a-zA-Z_]\w*)|(?:\b(?:echo|function|return|class|public|protected|private|static|new|if|else|elseif|switch|foreach|extends|implements|interface|trait|use|namespace|try|catch|throw|true|false|null|Attribute|fn|array)\b)/g;
  let lastIndex = 0;
  const elements = [];
  let match;

  while ((match = regex.exec(codeText)) !== null) {
    if (match.index > lastIndex) {
      elements.push(<Text key={`text-${lastIndex}`} style={{ color: '#E5E7EB' }}>{codeText.substring(lastIndex, match.index)}</Text>);
    }
    const token = match[0];
    let color = '#E5E7EB'; 
    if (token.startsWith('//') || token.startsWith('--') || token.startsWith('#')) color = '#4ADE80';
    else if (token.startsWith('"') || token.startsWith("'")) color = '#FDE047';
    else if (token.startsWith('$')) color = '#C084FC';
    else color = '#60A5FA';

    elements.push(<Text key={`token-${match.index}`} style={{ color }}>{token}</Text>);
    lastIndex = regex.lastIndex;
  }
  if (lastIndex < codeText.length) {
    elements.push(<Text key={`text-${lastIndex}`} style={{ color: '#E5E7EB' }}>{codeText.substring(lastIndex)}</Text>);
  }
  return elements;
};

export default function Module19DetailScreen() { 
  const insets = useSafeAreaInsets();
  const [isSpeaking, setIsSpeaking] = useState(false);
  const [copiedId, setCopiedId] = useState(null);

  useEffect(() => { return () => { Speech.stop(); }; }, []);

  const handleReadAloud = () => {
    if (isSpeaking) {
      Speech.stop();
      setIsSpeaking(false);
    } else {
      setIsSpeaking(true);
      Speech.speak("Module 19: Mail. Learn how to configure mail drivers, create mailables, send markdown emails, and queue background email jobs in Laravel.", { rate: 0.9 });
      lessonContent.forEach((lesson, index) => {
        let topicSpeech = `Topic ${index + 1}: ${lesson.title}. ${lesson.content}`;
        if (lesson.code) topicSpeech += ` Code snippet: ${lesson.code}`;
        const isLastItem = index === lessonContent.length - 1;
        Speech.speak(topicSpeech, {
          rate: 0.9,
          onDone: isLastItem ? () => setIsSpeaking(false) : undefined,
          onStopped: isLastItem ? () => setIsSpeaking(false) : undefined,
          onError: isLastItem ? () => setIsSpeaking(false) : undefined,
        });
      });
    }
  };

  const copyToClipboard = async (text, id) => {
    await Clipboard.setStringAsync(text);
    setCopiedId(id);
    setTimeout(() => setCopiedId(null), 2000);
  };

  return (
    <View className="flex-1 bg-gray-50">
      <StatusBar barStyle="light-content" backgroundColor="#FF3B30" />
      <View style={{ paddingTop: insets.top }} className="bg-[#FF3B30] rounded-b-[48px] shadow-lg z-20">
        <SafeAreaView>
          <View className="pt-4 pb-10 px-6">
            <View className="flex-row items-center justify-between mb-6">
              <TouchableOpacity onPress={() => { Speech.stop(); router.back(); }} className="w-10 h-10 bg-black/10 rounded-full items-center justify-center">
                <Ionicons name="arrow-back" size={24} color="white" />
              </TouchableOpacity>
              <Text className="text-white/80 text-sm font-semibold uppercase tracking-wider">Module 19</Text>
              <View className="w-10 h-10" />
            </View>
            <View>
              <Text className="text-white text-3xl font-extrabold tracking-wide mb-2">Mail</Text>
              <Text className="text-blue-100 text-sm leading-relaxed mb-4">Learn email configuration, mailable classes, markdown templates, and background queue dispatching.</Text>
              <View className="flex-row items-center space-x-3">
                <View className="flex-row items-center bg-black/10 px-3 py-1.5 rounded-lg border border-white/10">
                  <Ionicons name="mail" size={14} color="#DBEAFE" />
                  <Text className="text-white text-xs font-semibold ml-1.5">{lessonContent.length} Topics</Text>
                </View>
                <TouchableOpacity onPress={handleReadAloud} className={`flex-row items-center px-3 py-1.5 rounded-lg border ${isSpeaking ? 'bg-white border-white' : 'bg-black/10 border-white/10'}`}>
                  <Ionicons name={isSpeaking ? "stop-circle" : "volume-high"} size={14} color={isSpeaking ? "#FF3B30" : "#DBEAFE"} />
                  <Text className={`${isSpeaking ? 'text-[#FF3B30]' : 'text-white'} text-xs font-semibold ml-1.5`}>{isSpeaking ? "Stop Audio" : "Listen"}</Text>
                </TouchableOpacity>
              </View>
            </View>
          </View>
        </SafeAreaView>
      </View>
      <ScrollView className="flex-1 px-5 pt-8" showsVerticalScrollIndicator={false}>
        {lessonContent.map((lesson, index) => (
          <View key={lesson.id} className="bg-white p-5 mb-6 rounded-3xl border border-gray-100 shadow-sm">
            <View className="flex-row items-center mb-3">
              <View className="w-10 h-10 rounded-full items-center justify-center mr-3" style={{ backgroundColor: `${lesson.color}15` }}>
                <Ionicons name={lesson.icon} size={20} color={lesson.color} />
              </View>
              <View className="flex-1">
                <Text className="text-gray-400 text-[10px] font-bold uppercase tracking-wider mb-0.5">Topic {index + 1}</Text>
                <Text className="text-gray-800 font-bold text-base">{lesson.title}</Text>
              </View>
            </View>
            <Text className="text-gray-500 text-sm leading-relaxed">{lesson.content}</Text>
            {lesson.code && (
              <View className="bg-gray-800 p-4 rounded-xl mt-4 flex-row items-start justify-between">
                <Text className="font-mono text-xs leading-relaxed flex-1 mr-3">{renderHighlightedCode(lesson.code)}</Text>
                <TouchableOpacity onPress={() => copyToClipboard(lesson.code, lesson.id)} className="bg-white/10 p-2 rounded-lg" disabled={copiedId === lesson.id}>
                  <Ionicons name={copiedId === lesson.id ? "checkmark" : "copy-outline"} size={16} color={copiedId === lesson.id ? "#10B981" : "#9CA3AF"} />
                </TouchableOpacity>
              </View>
            )}
          </View>
        ))}
        <View className="h-12" />
      </ScrollView>
    </View>
  );
}